...
class SettingsController extends Controller
{
    ...
    public function index(Request $request,$name)
    {
        ...
        if($name=='license') {
            ...
            if($license) {
                ...
                $ch = curl_init('https://www.port-pro.online/api/license/validate');
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $post);
                $response = curl_exec($ch);
    ...
    public function restore_backup(Request $request,$name)
    {
        ...
        Process::run("mysql -u '" . env('DB_USERNAME') . "' --password='" . env('DB_PASSWORD') . "' Port_Pro_Panel < /var/www/html/app/storage/backup/" . $name);
    ...
    public function make_backup()
    {
        ...
        Process::run("mysqldump -u '" .env('DB_USERNAME'). "' --password='" .env('DB_PASSWORD'). "' Port_Pro_Panel > /var/www/html/app/storage/backup/Port-Pro-Panel-".$date.".sql");
        return redirect()->intended(route('settings', ['name' => 'backup']));
    }
