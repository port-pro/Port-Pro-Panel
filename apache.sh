...
echo -e "${YELLOW}************ Select Port_Pro_Panel Version Apache Web Server************"
echo -e "${GREEN}  1)Port-Pro-Panel v3.8.2"
echo -e "${GREEN}  2)Port-Pro-Panel v3.8.1"
echo -e "${GREEN}  3)Port-Pro-Panel v3.8.0"
echo -e "${GREEN}  4)Port-Pro-Panel v3.7.9"
echo -e "${GREEN}  5)Port-Pro-Panel v3.7.8"
echo -e "${GREEN}  6)Port-Pro-Panel v3.7.7"
echo -e "${GREEN}  7)Port-Pro-Panel v3.7.6"
echo -ne "${GREEN}\nSelect Version : ${ENDCOLOR}" ;read n
if [ "$n" != "" ]; then
if [ "$n" == "1" ]; then
linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-8-2
fi
if [ "$n" == "2" ]; then
linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-8-1
fi
if [ "$n" == "3" ]; then
linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-8-0
fi
if [ "$n" == "4" ]; then
linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-7-9
fi
if [ "$n" == "5" ]; then
linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-7-8
fi
if [ "$n" == "6" ]; then
linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/37
fi
if [ "$n" == "7" ]; then
linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/xpanel
fi
else
linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-8-2
fi
...
wait
clear
# Random port number generator to prevent Port-Pro-Panel detection by potential attackers
randomPort=""
...
sudo bash -c "$(curl -Lfo- https://raw.githubusercontent.com/Port-Pro-Panel/Nethogs-Json-main/master/install.sh)"
sudo mysql -e "create database Port_Pro_Panel;" &
wait
...
then
 sudo mysql -e "USE Port_Pro_Panel; UPDATE admins SET username = '${adminusername}' where permission='admin';"
 sudo mysql -e "USE Port_Pro_Panel; UPDATE admins SET password = '${adminpassword}' where permission='admin';"
 sudo mysql -e "USE Port_Pro_Panel; UPDATE settings SET ssh_port = '${port}' where id='1';"
else
sudo mysql -e "USE Port_Pro_Panel; INSERT INTO admins (username, password, permission, credit, status) VALUES ('${adminusername}', '${adminpassword}', 'admin', '', 'active');"
sudo home_url=$protcohttp://${defdomain}:$sshttp
sudo mysql -e "USE Port_Pro_Panel; INSERT INTO settings (ssh_port, tls_port, t_token, t_id, language, multiuser, ststus_multiuser, home_url) VALUES ('${port}', '444', '', '', '', 'active', '', '${home_url}');"
fi
...
wait
sudo curl -o /root/Port-Pro-Panel.sh https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/cli.sh
sudo wget -4 -O /usr/local/bin/Port-Pro-Panel https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/cli.sh
sudo chmod +x /usr/local/bin/Port-Pro-Panel 
...
DEFAULT_PANEL_DIRECT=Port-Pro-Panel
...
echo -e "************ Port-Pro-Panel ************ \n"
echo -e "Port-Pro-Panel Link : $protcohttp://${defdomain}:$sshttp/login"
