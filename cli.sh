...
sshport=$(mysql -N -e "use Port_Pro_Panel; select ssh_port from settings where id='1';")
...
function show_menu() {
    clear
    echo "Detail Port_Pro_Panel"
    echo "------------------"
    echo "Username: $adminuser"
    echo "Password: $def_pw"
    echo "SSH PORT: $sshport"
    echo "SSH PORT TLS: $ssh_tls_port"
    echo "Port_Pro_Panel Link: \"http://$domain:$def_port/$def_cp/login\""
    echo ""
    echo "Port_Pro_Panel CLI Menu"
    echo "------------------"
    
    echo "${BLUE}1. Change Username AND Password"
    echo "${RED}2. Change Port SSH"
    echo "${BLUE}3. Change Port SSH TLS"
    echo "${RED}4. Update Port_Pro_Panel Nginx Web Server"
    echo "${BLUE}5. Remove Port_Pro_Panel"
    echo "${RED}6. Remove All Admin Port_Pro_Panel"
    ...
    echo "${BLUE}13. Update Port_Pro_Panel Apache Web Server"
...
function select_option() {
    ...
    case $choice in
        1)
            ...
            mysql -e "USE Port_Pro_Panel; UPDATE admins SET username = '${username}' where id='1';"
            mysql -e "USE Port_Pro_Panel; UPDATE admins SET password = '${password}' where id='1';"
            ;;
        2)
...
connect = 0.0.0.0:$sshport
            ...
            mysql -e "USE Port_Pro_Panel; UPDATE settings SET tls_port = '${tlsport}' where id='1';"
            reboot
            ;;
        4)
            bash <(curl -Ls https://github.com/port-pro/Port-Pro-Panel/raw/master/install.sh --ipv4)
            ;;
        5)
        ...
        6)
       mysql -e "USE Port_Pro_Panel; TRUNCATE TABLE admins;"
...
EOF
            ;;
            8)
            bash <(curl -Ls https://github.com/port-pro/Port-Pro-Panel/raw/master/block_iran.sh --ipv4)
            ;;
            9)
             sudo iptables -F
            ;;
            10)
            bash <(curl -Ls https://raw.githubusercontent.com/port-pro/Dropbear-ssh/main/install.sh)
            ;;
            11)
            bash <(curl -Ls https://www.port-pro.online/wp-install.sh --ipv4)
            ;;
            12)
