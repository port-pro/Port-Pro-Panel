...
cp /var/www/html/example/index.php /var/www/
  ...
  if dpkg-query -W -f='${Status}' mariadb-server 2>/dev/null | grep -q "install ok installed"; then
    adminuser=$(mysql -N -e "use Port_Pro_Panel; select username from admins where permission='admin';")
    adminpass=$(mysql -N -e "use Port_Pro_Panel; select username from admins where permission='admin';")
    ssh_tls_port=$(mysql -N -e "use Port_Pro_Panel; select tls_port from settings where id='1';")
  fi
...
wellcomeINSTALL() {
  echo -e "${YELLOW}************ Select Port_Pro_Panel Version Nginx Web Server************"
  echo -e "${GREEN}  1)Port_Pro_Panel v3.9.4"
  echo -e "${GREEN}  2)Port_Pro_Panel v3.9.1"
  echo -e "${GREEN}  3)Port_Pro_Panel v3.8.7"
  echo -e "${GREEN}  4)Port_Pro_Panel v3.8.6"
  echo -e "${GREEN}  5)Port_Pro_Panel v3.8.5"
  echo -e "${GREEN}  6)Port_Pro_Panel v3.7.9"
  echo -ne "${GREEN}\nSelect Version : ${ENDCOLOR}"
  read n
  if [ "$n" != "" ]; then
    if [ "$n" == "1" ]; then
      linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-9-4
    fi
    if [ "$n" == "2" ]; then
      linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-9-1
    fi
    if [ "$n" == "3" ]; then
      linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-8-7
    fi
    if [ "$n" == "4" ]; then
      linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-8-6
    fi
    if [ "$n" == "5" ]; then
      linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-8-5
    fi
    if [ "$n" == "6" ]; then
      linkd=https://api.github.com/repos/port-pro/Port-Pro-Panel/releases/tags/v3-7-9
    fi
...
EOF
    ...
    echo "=================  Port_Pro_Panel OpenSSL ======================"
    country=EG
    state=EGYPT
    locality=EGYPT
    organization=Port-Pro
    organizationalunit=Port-Pro
    commonname=www.port-pro.online
    email=admin@port-pro.online
    ...
    wait
    sudo wget -4 -O /usr/local/bin/cronx https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/cronx
    chmod +x /usr/local/bin/cronx
    sudo wget -4 -O /usr/local/bin/cronxfixed https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/cronxfixed
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/ioncube.sh --ipv4)
    wait
    ...
    # Random port number generator to prevent Port_Pro_Panel detection by potential attackers
    randomPort=""
    ...
    read udpport
    sudo bash -c "$(curl -Ls https://raw.githubusercontent.com/port-pro/Nethogs-Json-main/master/install.sh --ipv4)"
...
EOF
    ...
    sudo wget -q -O /usr/local/bin/wss https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/wss
    sudo chmod +x /usr/local/bin/wss
    sudo wget -q -O /usr/local/bin/wssd https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/wssd
...
checkDATABASE() {
  mysql -e "create database Port_Pro_Panel;" &
  wait
  ...
  if [ -n "$adminuser" -a "$adminuser" != "NULL" ]; then
    mysql -e "USE Port_Pro_Panel; UPDATE admins SET username = '${adminusername}' where permission='admin';"
    mysql -e "USE Port_Pro_Panel; UPDATE admins SET password = '${adminpassword}' where permission='admin';"
    mysql -e "USE Port_Pro_Panel; UPDATE settings SET ssh_port = '${port}' where id='1';"
    ...
    mysql -e "USE Port_Pro_Panel; INSERT INTO admins (username, password, permission, credit, status) VALUES ('${adminusername}', '${adminpassword}', 'admin', '', 'active');"
    home_url=$protcohttp://${defdomain}:$sshttp
    mysql -e "USE Port_Pro_Panel; INSERT INTO settings (ssh_port, tls_port, t_token, t_id, language, multiuser, ststus_multiuser, home_url) VALUES ('${port}', '444', '', '', '', 'active', '', '${home_url}');"
  fi
}
moreCONFIG() {
...
ENDOFFILE
  ...
  curl -o /root/Port_Pro_Panel.sh https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/cli.sh
  sudo wget -4 -O /usr/local/bin/Port_Pro_Panel https://raw.githubusercontent.com/port-pro/Port-Pro-Panel/master/cli.sh
  chmod +x /usr/local/bin/Port_Pro_Panel
...
endINSTALL() {
  echo -e "************ Port_Pro_Panel ************ \n"
  echo -e "Port_Pro_Panel Link : $protcohttp://${defdomain}:$sshttp/login"
